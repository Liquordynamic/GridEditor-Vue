...
class NHGridNode:
    ...
    def __init__(self, extent: list[float], id: int, height: float, x_min_percent: list[int], y_min_percent: list[int], x_max_percent: list[int], y_max_percent: list[int]):
...
class NHGridEdge:
    ...
    def __init__(self, extent: list[float], id: int,  height: float, adjacent_grids: list[NHGridNode | None], min_percent: list[int], max_percent: list[int], edge_code: int):
...
class NHGridHelper:
    ...
    def export(self, output_path: str, dem_path: str = '', invalid_data: float = -9999):
        ...
        for id in self.grids:
            ...
            center = ', '.join(map(str, [ *center_point, grid.height if grid.height!= invalid_data else sampler.sampling(*center_point) ]))
        ...
        for id in self.edges:
            ...
            center = ', '.join(map(str, [ *center_point, edge.height if edge.height != invalid_data else sampler.sampling(*center_point) ]))
