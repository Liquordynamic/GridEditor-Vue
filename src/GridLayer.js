...
export default class GridLayer {
    ...
    constructor(options) {
        ...
        */
        ...
        this.subdivideRules.push(...options.subdivideRules)
        ...
        this.edgeRecoder = new GridEdgeRecoder(options._edgeProperties)
        ...
        this.gui.domElement.parentElement.classList.add('overlay')
    ...
    set currentType(type) {
        ...
        if (type === this.EDITOR_TYPE) {
            this._vueDiv.style.display = 'block'
            ...
            this._vueDiv.style.display = 'none'
            // Change event handlers
    ...
    */
    ...
    serialize() {
         ...
         *      grids: { id: number, height:number, xMinPercent: [ number, number ], yMinPercent: [ number, number ], xMaxPercent: [ number, number ], yMaxPercent: [ number, number ] }[]
         *      edges: { id: number, height:number, adjGrids: [ number | null, number | null ], minPercent: [ number, number ], maxPercent: [ number, number ], edgeCode: number }[]
         * }}
         */
    ...
    async init() {
        ...
        // vue-Instance mount /////////////////////////////////////////////
        const update = (data) => {
            data.edgeValues.forEach((value, key) => {
                this.edgeRecoder.getEdgeByKey(key).height = value
            })
        }
        this._hittingGrid = ref(null);
        this._vueDiv = document.createElement('div')
        this._vueDiv.style.position = 'absolute'
        this._vueDiv.style.left = '60px'
        this._vueDiv.style.top = '60px'
        this._vueDiv.style.display = 'none'
        const vueEditor = createApp(PropertyEditor, {
            grid: this._hittingGrid,
            update,
            getEdge: (edgeKey) => {
                return this.edgeRecoder.getEdgeByKey(edgeKey)
            }
        })
        vueEditor.mount(this._vueDiv)
        document.body.appendChild(this._vueDiv)
